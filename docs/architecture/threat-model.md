...
The browser talks to the Next.js web application. The web application and worker talk to
local PostgreSQL. Phase 1 has no outbound provider calls and no public deployment.
Participant and reviewer sessions cross the browser/server trust boundary. Synthetic
receipt text crosses an untrusted-input boundary.
...
Only synthetic records belong in Phase 1. Email is collected for early access, consent
is explicit, analytics use internal identifiers, and raw input is not placed in log or
analytics properties. Participant deletion removes related sessions, submissions, jobs,
results, consent, payment signals, and analytics through database cascades or an
